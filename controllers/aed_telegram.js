...
const {extractNumber} = require("../function.js");
const {Aed} = require("../models");
const { response } = require("express");

const getAED = async () => {
    const pervData = await Aed.findAll({
        limit:1,
        order: [['id','DESC']],
        attributes: ['telegramTime']
    }) 

    const prevTime = pervData[0]?.telegramTime
    ...
    try {
        ...
        await page.goto('https://t.me/s/RateUAEonline', { waitUntil: 'domcontentloaded',timeout:60000 });
        ...
        const fetchData = await page.evaluate((prevTime) => {
            ...
            for (let messageWrap of messageWraps) {
                ...
                if (postText.includes("#RateUAEonline")) {
        ...
        for (const item of fetchData.reverse()) {
            const  number  = extractNumber(item.postText);
            const body = {
